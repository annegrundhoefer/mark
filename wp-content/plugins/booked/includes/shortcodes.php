			...
			if (!isset($atts['remove_wrapper'])): echo '<div id="booked-profile-page" class="booked-shortcode">'; endif;
				...
				echo '<div class="booked-profile-appt-list">';
					...
					foreach($appointments_array as $appt):
						...
						$status = ($appt['status'] !== 'publish' ? __('pending','booked') : __('approved','booked'));
						$status_class = $appt['status'] !== 'publish' ? 'pending' : 'approved';
						
						echo '<span class="appt-block bookedClearFix '.$status_class.'" data-appt-id="'.$appt['post_id'].'">';
							if (!$historic):
							...
							if (!$historic):
								if ($appt_date_time >= $date_to_compare || !get_option('booked_hide_google_link')):
									echo '<div class="booked-cal-buttons">';
										...
										if ( apply_filters('booked_shortcode_appointments_allow_cancel', true, $appt['post_id']) ) {
